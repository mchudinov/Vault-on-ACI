...
locals {
    vault_name = "vault-${random_integer.id.result}"
    resource_group_name = "rg-${var.prefix}-${random_integer.id.result}"
    storage_account_name = "sa${var.prefix}${random_integer.id.result}"
    key_vault_name = "kv-${var.prefix}-${random_integer.id.result}"
    user_identity_name = "${var.prefix}${random_integer.id.result}"
}
...
az container create --resource-group ${azurerm_resource_group.vault.name} \
  --name ${local.vault_name} --image hashicorp/vault-enterprise:latest \
